...
def imshow_det_bboxes(img,
    ...
    if segms is not None:
        mask_palette = get_palette(mask_color, len(labels))
        # colors = [mask_palette[label] for label in labels]
        # mask_palette = get_palette(mask_color, 100)
        # colors = [mask_palette[np.random.randint(0, 100)] for label in labels]
        colors = [mask_palette[i] for i in range(len(labels))]
        # color_mask=mask_colors[np.random.randint(0,80)]
