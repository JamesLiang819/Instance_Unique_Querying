...
class MaskFormerHead_contrast(AnchorFreeHead):
    ...
    def loss_contrast(self, all_cls_scores, all_mask_preds, gt_labels_list,
             gt_masks_list, img_metas):
        """Loss function.
        ...
        loss_dict = dict()
        # loss from the last decoder layer
        # loss_dict['loss_cls_contrast'] = losses_cls[-1]
        loss_dict['loss_mask_contrast'] = losses_mask[-1]/3
        loss_dict['loss_dice_contrast'] = losses_dice[-1]/3
        # loss from other decoder layers
        num_dec_layer = 0
        for loss_cls_i, loss_mask_i, loss_dice_i in zip(
                losses_cls[:-1], losses_mask[:-1], losses_dice[:-1]):
            # loss_dict[f'd{num_dec_layer}.loss_cls_contrast'] = loss_cls_i
            loss_dict[f'd{num_dec_layer}.loss_mask_contrast'] = loss_mask_i/(50/(num_dec_layer+1))
            loss_dict[f'd{num_dec_layer}.loss_dice_contrast'] = loss_dice_i/(50/(num_dec_layer+1))
            num_dec_layer += 1
        return loss_dict
    ...
    def forward_train(self,
        ...
        self.feat_bank.append(feats)
        contrast_feats=self.feat_bank[0]
        # forward
        all_cls_scores, all_mask_preds , all_cls_scores_contrast, all_mask_preds_contrast= self(feats,contrast_feats, img_metas)
        # print(all_mask_preds[0].shape,all_mask_preds_contrast[0].shape)

        # preprocess ground truth
        gt_labels, gt_masks = self.preprocess_gt(gt_labels, gt_masks,
                                                 gt_semantic_seg)
        self.gt_bank.append([gt_labels,gt_masks])
        # loss
        losses = self.loss(all_cls_scores, all_mask_preds, gt_labels, gt_masks,
                           img_metas)

        contrast_gt_labels,contrast_gt_masks=self.gt_bank[0]
        # contrast_gt_list=[]
        # contrast_gt_label_list=[]
        # print(len(gt_labels),len(contrast_gt_labels))
        # for i in range(len(contrast_gt_masks)):
        #     # temp_weight=[]
        #     temp=[]
        #     temp_label=[]
        #     n=min(len(gt_labels[i]),len(contrast_gt_labels[i]))
        #     if n == 0:
        #         continue
        #         # print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
        #     for j in range(n):
        #         # print(gt_labels[i].shape,contrast_gt_labels[i].shape)
        #         if gt_labels[i][j]==contrast_gt_labels[i][j]:
        #             temp.append(contrast_gt_masks[i][j].unsqueeze(0))
        #         else:
        #             temp.append(0*contrast_gt_masks[i][j].unsqueeze(0))
        #         temp_label.append(contrast_gt_labels[i])
        #     contrast_gt_list.append(torch.cat(temp))
        #     contrast_gt_label_list.append(torch.cat(temp_label))
        # print(len(contrast_gt_masks),len(contrast_gt_list))
        # print(contrast_gt_masks[0].shape,contrast_gt_list[0].shape)
        # print(gt_labels[0].shape,contrast_gt_labels[0].shape,all_cls_scores_contrast[0].shape,all_mask_preds_contrast[0].shape,contrast_gt_masks[0].shape)
        contrast_losses = self.loss_contrast(all_cls_scores_contrast, all_mask_preds_contrast, contrast_gt_labels, contrast_gt_masks,
                    img_metas)
        ct=1
        for i in range(1,len(self.feat_bank)-1):
            contrast_feats=self.feat_bank[i]
            contrast_gt_labels,contrast_gt_masks=self.gt_bank[i]
            all_cls_scores, all_mask_preds , all_cls_scores_contrast, all_mask_preds_contrast= self(feats,contrast_feats, img_metas)
            temp = self.loss_contrast(all_cls_scores_contrast, all_mask_preds_contrast, contrast_gt_labels, contrast_gt_masks,
                    img_metas)
            ct+=1
            for key,value in temp.items():
                contrast_losses[key]=contrast_losses[key]+temp[key]
         
        # print(losses)
        for key,value in contrast_losses.items():
            losses[key]=contrast_losses[key]/ct
        
        if len(self.gt_bank)>num_memory_bank:
            self.gt_bank.pop(0)
    ...
    def simple_test(self, feats, img_metas, **kwargs):
        ...
        """
        all_cls_scores, all_mask_preds,_,_ = self(feats,feats, img_metas)
