...
from .dynamic_mask_head_contrast_test import build_dynamic_mask_head
...
class CondInst_contrast_test(nn.Module):
    """
    ...
    def __init__(self, cfg):
        ...
        # self.contrast_controller = nn.Conv2d(
        #     in_channels, self.mask_head.num_gen_params,
        #     kernel_size=3, stride=1, padding=1
        # )
        # torch.nn.init.normal_(self.contrast_controller.weight, std=0.01)
        # torch.nn.init.constant_(self.contrast_controller.bias, 0)
    ...
    def forward(self, batched_inputs):
        # print(n)
        batch_original_images = [x["image"].to(self.device) for x in batched_inputs]
        n=len(batch_original_images)
        # print(n)
        original_images=batch_original_images[:int(n/2)]
        contrast_images=batch_original_images[int(n/2):]
        # contrast_images=[x["image"].to(self.device) for x in batched_inputs]
        # print(original_images[0].shape,contrast_images[0].shape)
        if n == 1:
            original_images=batch_original_images
            contrast_images=batch_original_images
        # normalize images
        images_norm = [self.normalizer(x) for x in original_images]
        contrast_images_norm = [self.normalizer(x) for x in contrast_images]
        images_norm = ImageList.from_tensors(images_norm, self.backbone.size_divisibility)
        contrast_images_norm = ImageList.from_tensors(contrast_images_norm, self.backbone.size_divisibility)

        features = self.backbone(images_norm.tensor)
        contrast_features = self.backbone(contrast_images_norm.tensor)


        if "instances" in batched_inputs[0]:
            # gt_instances = [x["instances"].to(self.device) for x in batched_inputs]
            # n=len(batched_inputs)
            batch_gt_instances = [x["instances"].to(self.device) for x in batched_inputs]
            gt_instances = batch_gt_instances[:int(n/2)]
            # aug_gt_instances = [x["instances"].to(self.device) for x in batched_inputs]
            # aug_gt_instances = batch_gt_instances[int(n/2):]
            # print(aug_gt_instances.shape)
            contrast_gt_instances=batch_gt_instances[int(n/2):]
            # contrast_gt_instances = [x["instances"].to(self.device) for x in batched_inputs]
            if self.boxinst_enabled:
                ...
                self.add_bitmasks(contrast_gt_instances, contrast_images_norm.tensor.size(-2), contrast_images_norm.tensor.size(-1))

        else:
            gt_instances = None
            contrast_gt_instances = None
        mask_feats, sem_losses = self.mask_branch(features, gt_instances)
        contrast_mask_feats, contrast_sem_losses = self.mask_branch(contrast_features, contrast_gt_instances)
        # print(sem_losses,contrast_sem_losses)

        proposals, proposal_losses = self.proposal_generator(
            images_norm, features, gt_instances, self.controller
        )
        contrast_proposals, contrast_proposal_losses = self.proposal_generator(
            contrast_images_norm, contrast_features, contrast_gt_instances, self.controller
        )
        # print(proposal_losses,contrast_proposal_losses)
        for key,value in contrast_proposal_losses.items():
            proposal_losses[key]+=value
            proposal_losses[key]/=2

        # print(proposals['instances'])
        # raise NameError("STOP!")
        if self.training:
            # print(len(gt_instances),len(contrast_gt_instances))
            mask_losses = self._forward_mask_heads_train(proposals, mask_feats, gt_instances,contrast_proposals,contrast_mask_feats,contrast_gt_instances)
            # contrast_mask_losses=self._forward_mask_heads_train(contrast_proposals, contrast_mask_feats, contrast_gt_instances,contrast_gt_instances,base_weights,base_biases,base_n_inst=base_n_inst)
            # contrast_mask_losses = self._forward_mask_heads_train(proposals, mask_feats, gt_instances)
            ...
            # losses.update(contrast_sem_losses)
            losses.update(proposal_losses)
            # losses.update(0*contrast_proposal_losses)
            # losses.update(contrast_proposal_losses)
            losses.update(mask_losses)
            # print(contrast_mask_losses)
            # losses.update(contrast_mask_losses)
            return losses
        else:
            pred_instances_w_masks = self._forward_mask_heads_test(proposals, mask_feats)
            # print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')

            # print(pred_instances_w_masks.shape)
            # test=[per_im.gt_bitmasks_full for per_im in gt_instances][0][1]
            # # print(test)
            # test=test.clone().detach()
            # test=test.to(torch.device('cpu')).numpy()*255
            # print(sum(test))
            # # print(original_images[0].shape)
            # plt.imshow(test)
            # plt.savefig('demo/condinst/1.jpg')
            # cv2.imwrite('demo/condinst/1.jpg',test)
            # raise NameError('!')
    ...
    def _forward_mask_heads_train(self, proposals, mask_feats, gt_instances,contrast_proposals,contrast_mask_feats,contrast_gt_instances):
        # prepare the inputs for mask heads
        pred_instances = proposals["instances"]
        contrast_pred_instances = contrast_proposals["instances"]
        # pred_instances= torch.cat(10*[pred_instances],dim=0)
        n=min(len(contrast_pred_instances),len(pred_instances))
        ...
        if self.max_proposals != -1:
            if self.max_proposals < n:
                inds = torch.randperm(n, device=mask_feats.device).long()
                ...
                inds = torch.randperm(n, device=mask_feats.device).long()
                pred_instances = pred_instances[inds[:n]]
...
#############################################################################################################################
        # contrast_pred_instances= torch.cat(10*[contrast_pred_instances],dim=0)
        assert (self.max_proposals == -1) or (self.topk_proposals_per_im == -1), \
            "MAX_PROPOSALS and TOPK_PROPOSALS_PER_IM cannot be used at the same time."
        if self.max_proposals != -1:
            if self.max_proposals < n:
                contrast_inds = torch.randperm(n, device=contrast_mask_feats.device).long()
                # logger.info("clipping proposals from {} to {}".format(
                #     len(contrast_pred_instances), self.max_proposals
                # ))
                contrast_pred_instances = contrast_pred_instances[contrast_inds[:self.max_proposals]]
            else:
                contrast_inds = torch.randperm(n, device=contrast_mask_feats.device).long()
                contrast_pred_instances = contrast_pred_instances[contrast_inds[:n]]
        elif self.topk_proposals_per_im != -1:
            num_images = len(contrast_gt_instances)

            contrast_kept_instances = []
            for im_id in range(num_images):
                contrast_instances_per_im = contrast_pred_instances[contrast_pred_instances.im_inds == im_id]
                if len(instances_per_im) == 0:
                    contrast_kept_instances.append(contrast_instances_per_im)
                    continue

                contrast_unique_gt_inds = contrast_instances_per_im.gt_inds.unique()
                contrast_num_instances_per_gt = max(int(self.topk_proposals_per_im / len(contrast_unique_gt_inds)), 1)

                for gt_ind in contrast_unique_gt_inds:
                    contrast_instances_per_gt = contrast_instances_per_im[contrast_instances_per_im.gt_inds == gt_ind]

                    if len(contrast_instances_per_gt) > contrast_num_instances_per_gt:
                        contrast_scores = contrast_instances_per_gt.logits_pred.sigmoid().max(dim=1)[0]
                        contrast_ctrness_pred = contrast_instances_per_gt.ctrness_pred.sigmoid()
                        contrast_inds = (contrast_scores * contrast_ctrness_pred).topk(k=contrast_num_instances_per_gt, dim=0)[1]
                        contrast_instances_per_gt = contrast_instances_per_gt[contrast_inds]

                    contrast_kept_instances.append(contrast_instances_per_gt)

            contrast_pred_instances = Instances.cat(contrast_kept_instances)

        contrast_pred_instances.mask_head_params = contrast_pred_instances.top_feats
#############################################################################################################################
        ...
        loss_mask = self.mask_head(
            ...
            pred_instances, gt_instances,contrast_mask_feats,contrast_pred_instances,contrast_gt_instances
        )
    ...
    def _forward_mask_heads_test(self, proposals, mask_feats):
        ...
        pred_instances_w_masks = self.mask_head(
            mask_feats, self.mask_branch.out_stride, pred_instances,None,mask_feats,pred_instances
        )

        return pred_instances_w_masks

    def add_bitmasks(self, instances, im_h, im_w,aug=False):
        for per_im_gt_inst in instances:
            ...
            if isinstance(per_im_gt_inst.get("gt_masks"), PolygonMasks):
                ...
                if not aug:
                    ...
                    # print(type(per_im_bitmasks[0]))
                else:
                    per_im_gt_inst.gt_bitmasks = torch.stack(per_im_bitmasks, dim=0)
                    per_im_gt_inst.gt_bitmasks_full = torch.stack(per_im_bitmasks_full, dim=0)
                    # per_im_gt_inst.gt_bitmasks = torch.stack(torch.flip(per_im_bitmasks,dims=[1]), dim=0)
                    # per_im_gt_inst.gt_bitmasks_full = torch.stack(torch.flip(per_im_bitmasks_full,dims=[1]), dim=0)
