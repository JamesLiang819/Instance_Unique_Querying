...
class SOLOv2_contrast(nn.Module):
    """
    ...
    def forward(self, batched_inputs):
        ...
        if n==1:
            ...
            equi_images = images.tensor
        else:
            images = self.preprocess_image(batched_inputs)
            equi_images = torch.flip(images.tensor,dims=[3])
        ...
        equi_features = self.backbone(equi_images)
        ...
        equi_ins_features = [equi_features[f] for f in self.instance_in_features]
        equi_ins_features = self.split_feats(equi_ins_features)
        equi_cate_pred, equi_kernel_pred = self.ins_head(equi_ins_features)

        # mask branch
        mask_features = [features[f] for f in self.mask_in_features]
        mask_pred = self.mask_head(mask_features)

        # equi_mask_features = [equi_features[f] for f in self.mask_in_features]
        # equi_mask_pred = self.mask_head(equi_mask_features)
        ...
        if self.training:
            ...
            equi_losses = self.equi_loss(equi_cate_pred, equi_kernel_pred, torch.flip(mask_pred,dims=[3]), targets)
            # print(losses)
            for key,value in equi_losses.items():
                losses[key]=0.3*equi_losses[key]
            
            for i in range(len(self.memory_bank)):
                contrast_mask_pred = self.memory_bank[i]
                contrast_losses = self.contrast_loss(cate_pred, kernel_pred, contrast_mask_pred, targets)
                if i == 0:
                    for key,value in contrast_losses.items():
                        losses[key]=0.3*contrast_losses[key]
                else:
                    for key,value in contrast_losses.items():
                        losses[key]+=0.3*contrast_losses[key]
            ...
            self.memory_bank.append(mask_pred.clone().detach())
    ...
    def contrast_loss(self, cate_preds, kernel_preds, ins_pred, targets):
        pass
        ins_label_list, cate_label_list, ins_ind_label_list, grid_order_list = targets
        # ins
        ins_labels = [torch.cat([ins_labels_level_img
        ...
        for input, target in zip(ins_pred_list, ins_labels):
            ...
            input=torch.where(input>0.18,zero,input)
            loss_ins.append(torch.mean(input))

        # if len(loss_ins) != 0:
        #     loss_ins_mean = torch.cat(loss_ins).mean()
        # else:
        #     loss_ins_mean = 0
        loss_ins = torch.mean(torch.tensor(loss_ins)) * self.ins_loss_weight * 0.1

        return {'contrast_loss': loss_ins}

    def equi_loss(self, cate_preds, kernel_preds, ins_pred, targets):
        pass
        ins_label_list, cate_label_list, ins_ind_label_list, grid_order_list = targets
        # ins
        ins_labels = [torch.cat([ins_labels_level_img
        ...
        for b_kernel_pred in kernel_preds:
            ...
            if len(b_mask_pred) == 0:
                ...
                b_mask_pred = torch.cat(b_mask_pred, 0)
            if b_mask_pred != None:
                b_mask_pred=torch.flip(b_mask_pred,dims=[2])
            ins_pred_list.append(b_mask_pred)
        ...
        loss_cate = self.focal_loss_weight * sigmoid_focal_loss_jit(flatten_cate_preds, flatten_cate_labels_oh,
                                    ...
                                    alpha=self.focal_loss_alpha,
                                    reduction="sum") / (num_ins + 1)
        return {'equi_loss_ins': loss_ins,
                'equi_loss_cate': loss_cate}
