...
def imresize(img,
    ...
    try:
        resized_img = cv2.resize(
            img, (size[0],size[1]), dst=out, interpolation=interp_codes[interpolation])
    except:
        try:
            print(img.shape[:], size)
            resized_img = cv2.resize(
                img.transpose(1,2,0), size, dst=out, interpolation=interp_codes[interpolation]).transpose(2,0,1)
        except:
            print(img.shape[:], size)
            resized_img = cv2.resize(
                img.transpose(0,2,1), size, dst=out, interpolation=interp_codes[interpolation]).transpose(0,2,1)
